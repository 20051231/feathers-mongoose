...
class Service {
  ...
  _find (params, count, getFilter = filterQuery) {
    ...
    if(params.sessionStore && params.sessionStore.dbSession) {
      q.session(params.sessionStore.dbSession);
    }
  ...
  _get (id, params = {}) {
    ...
    if(params.sessionStore && params.sessionStore.dbSession) {
      modelQuery.session(params.sessionStore.dbSession);
    }
  ...
  create (data, params) {
    ...
    const model = this.discriminators[discriminator] || this.Model;
    return model.create([data], { session: params.sessionStore && params.sessionStore.dbSession })
      .then(([result]) => {
        if (this.lean) {
