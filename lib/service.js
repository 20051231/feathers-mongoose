...
class Service extends AdapterService {
  ...
  _find (params = {}) {
    ...
    if(params.sessionStore && params.sessionStore.session) {
      q.session(params.sessionStore.session);
    }
    ...
    if (paginate && paginate.default) {
      return model.where(query)[this.useEstimatedDocumentCount ? 'estimatedDocumentCount' : 'countDocuments']()
        .session(params.sessionStore && params.sessionStore.session).exec().then(executeQuery);
    }
  ...
  _get (id, params = {}) {
    ...
    if(params.sessionStore && params.sessionStore.session) {
      modelQuery.session(params.sessionStore.session);
    }
  ...
  _create (_data, params = {}) {
    ...
    return model.create(data, { session: params.sessionStore && params.sessionStore.session }).then(results => {
      if ($populate) {
  ...
  _remove (id, params = {}) {
    ...
    return this._getOrFind(id, params).then(data => {
        const modelQuery = this.Model
          .deleteMany(query, params.mongoose);
          if(params.sessionStore && params.sessionStore.session) {
            modelQuery.session(params.sessionStore.session);
          }
        
          return modelQuery.lean(this.lean)
          .exec()
          .then(() => data)
