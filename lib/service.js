...
class Service {
  ...
  create (data, params) {
    ...
    const model = this.discriminators[discriminator] || this.Model;
    return model.create([data], { session: params.sessionStore && params.sessionStore.session })
      .then(([result]) => {
        if (this.lean) {
    ...
    try {
      return ids
        .then(idList => {
          ...
          if(params.sessionStore && !findParams.sessionStore) {
            Object.defineProperty(findParams, 'sessionStore', {
              value: params.sessionStore,
              enumerable: false,
            })
          }
